 ...
 */
export function rowToCard(row: string[], now: string, hasAssetsFn: (id: string) => boolean): CardSummary | null {
    ...
    const gradingRaw = row
        .slice(graderIdx + 1, certIdx)
        .map(trim)
        .filter(Boolean)
    const { overallGrade, autoGrade, centering, corners, edges, surface } = parseGradingValues(gradingRaw)
...
export function parseSheetCSV(csvText: string, hasAssetsFn: (id: string) => boolean): CardSummary[] {
