...
import { applyOverrides } from '@/lib/cardOverrides'
...
function hasAssets(id: string): boolean {
    ...
    return fs.existsSync(path.join(dir, 'front.png')) && fs.existsSync(path.join(dir, 'back.png'))
}
    ...
    if (SHEET_ID) {
        try {
            ...
            if (res.ok) {
                ...
                // Persist the Sheets-derived data first, then merge overrides at read
                // time only — so card-overrides.json never gets baked into the cache.
                writeCache(manifest)
                applyOverrides(manifest.cards)
                return manifest
            }
